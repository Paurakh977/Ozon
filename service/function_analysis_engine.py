...
class FunctionAnalysisEngine:
                    ...
                    if getattr(root, 'is_real', True):
                        ...
                        if self._in_domain(root_val, domain):
                            ...
                            if check_val is not None and abs(check_val) < 1e-10:
                                ...
                                eps = 1e-6
                                left_val = self._safe_eval(expr, root_val - eps) if self._in_domain(root_val - eps, domain) else None
                                right_val = self._safe_eval(expr, root_val + eps) if self._in_domain(root_val + eps, domain) else None
                                ...
                                if left_val is not None and right_val is not None:
                                    ...
                                    if left_val * right_val < 0:
                                        ...
                                        wide_eps = 0.3
                                        far_left = self._safe_eval(expr, root_val - wide_eps)
                                        far_right = self._safe_eval(expr, root_val + wide_eps)
                                        if (far_left is not None and abs(far_left) < 1e-8 and
                                            self._in_domain(root_val - wide_eps, domain)):
                                            is_real_zero = False  # flat zero — skip it
                                        elif (far_right is not None and abs(far_right) < 1e-8 and
                                              self._in_domain(root_val + wide_eps, domain)):
                                            is_real_zero = False  # flat zero — skip it
                                        else:
                                            is_real_zero = True   # isolated zero at cusp/endpoint
                                    elif left_val * right_val <= 0:  # covers == 0 case with sign change
                                        is_real_zero = True
                                ...
                                if is_real_zero:
                                    ...
                                    is_dup = any(abs(root_val - existing) < 0.01 for existing in found_roots)
                                    if not is_dup:
    ...
    def get_extrema(self, expr, domain):
        ...
        try:
            ...
            if self._roots_are_incomplete(roots) and self._has_infinite_oscillation(expr):
    ...
    def get_monotonicity(self, expr, domain, period=None):
        ...
        try:
            ...
            if suppress_infinity:
                self._log("Infinite oscillation detected - cannot determine monotonicity intervals")
                return intervals
            ...
            if oscillates_at_inf:
                ...
                fp_vals = [self._safe_eval(f_prime, t) for t in test_pts
                           if self._in_domain(t, domain)]
                fp_vals = [v for v in fp_vals if v is not None]
                if fp_vals and all(v >= -1e-9 for v in fp_vals):
                    oscillates_at_inf = False  # always non-negative → globally increasing
                elif fp_vals and all(v <= 1e-9 for v in fp_vals):
                    oscillates_at_inf = False  # always non-positive → globally decreasing
    ...
    def print_report(self, res):
        ...
        def format_extrema(pts, period, expr=None):
            ...
            if period is None:
                ...
                force_annotation = (expr is not None and
                                   self._oscillates_at_infinity(expr))
        ...
        print(f"Minima:         {format_extrema(res['Extrema']['minima'], period, res['Function'])}")
        print(f"Maxima:         {format_extrema(res['Extrema']['maxima'], period, res['Function'])}")
        print(f"Inflection pts: {format_extrema(res['Inflection Points'], period, res['Function'])}")
        ...
        if not res['Monotonicity']['increasing'] and not res['Monotonicity']['decreasing']:
            ...
            def print_intervals(ivals, period, label, expr=None):
                ...
                if period is None:
                    ...
                    is_single_global = (len(ivals) == 1 and
                                        ivals[0][0] == -sp.oo and ivals[0][1] == sp.oo)
                    if is_single_global:
            ...
            print_intervals(res['Monotonicity']['increasing'], period, "Increasing", expr=res['Function'])
            print_intervals(res['Monotonicity']['decreasing'], period, "Decreasing", expr=res['Function'])
