...
fn find_sign_changes<'py>(
    ...
    for i in 0..arr.len().saturating_sub(1) {
        ...
        if v1.is_finite() && v2.is_finite() {
            // Strict sign change (crosses zero)
            if v1 * v2 < 0.0 {
                changes.push(i);
            }
            // Touches zero — potential extremum or root
            else if v2.abs() < 1e-12 && v1.abs() > 1e-12 {
                changes.push(i);
            }
        }
    }
...
fn format_symbolic_value(val: f64) -> String {
    ...
    if (val - 2.0 * PI).abs() < 1e-8 {
        return "2*pi".to_string();
    }
    if (val + 2.0 * PI).abs() < 1e-8 {
        return "-2*pi".to_string();
    }
    if (val - PI / 2.0).abs() < 1e-8 {
        return "pi/2".to_string();
    }
    if (val + PI / 2.0).abs() < 1e-8 {
        return "-pi/2".to_string();
    }
    if (val - PI / 3.0).abs() < 1e-8 {
        return "pi/3".to_string();
    }
    if (val + PI / 3.0).abs() < 1e-8 {
        return "-pi/3".to_string();
    }
    if (val - PI / 4.0).abs() < 1e-8 {
        return "pi/4".to_string();
    }
    if (val + PI / 4.0).abs() < 1e-8 {
        return "-pi/4".to_string();
    }
    if (val - PI / 6.0).abs() < 1e-8 {
        return "pi/6".to_string();
    }
    if (val + PI / 6.0).abs() < 1e-8 {
        return "-pi/6".to_string();
    }
    ...
    if (val - sqrt3).abs() < 1e-8 {
        return "sqrt(3)".to_string();
    }
    if (val + sqrt3).abs() < 1e-8 {
        return "-sqrt(3)".to_string();
    }
    if (val - sqrt2 / 2.0).abs() < 1e-8 {
        return "sqrt(2)/2".to_string();
    }
    if (val + sqrt2 / 2.0).abs() < 1e-8 {
        return "-sqrt(2)/2".to_string();
    }
    if (val - sqrt3 / 2.0).abs() < 1e-8 {
        return "sqrt(3)/2".to_string();
    }
    if (val + sqrt3 / 2.0).abs() < 1e-8 {
        return "-sqrt(3)/2".to_string();
    }
...
fn fast_math_rs(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add(
        "__doc__",
        "Fast numerical computation module for domain/range analysis",
    )?;
