...
def super_fast_limit(expr, n, prof: Profiler):
    ...
    def snap_limit(L):
        ...
        if L is not None and L.is_number and not L.has(sp.Limit):
            try:
                v = float(L)
                if abs(v - 1.0) < 1e-9:  return sp.S(1)
                if abs(v)       < 1e-9:  return sp.S(0)
                if abs(v + 1.0) < 1e-9:  return sp.S(-1)
            except Exception:
                pass
        return L
    ...
    has_n_exp = any(
        isinstance(arg, sp.Pow) and arg.exp.has(n)
...
def check_sequence_convergence(expr, n, py_func=None):
    ...
    has_n_root = any(
        p.exp.has(n) and sp.limit(p.exp, n, sp.oo) == 0
    ...
    if (expr.has((-1) ** n)
        ...
        if (L_even is not None and L_odd is not None
                and not L_even.has(sp.Limit) and not L_odd.has(sp.Limit)):
            if L_even == L_odd:
...
def check_series_convergence(expr, n, start_idx: int = 1, py_func=None):
    ...
    try:
        ...
        is_oscillatory = (
            expr.has((-1) ** n) or expr.has((-1) ** (n + 1)) or
            expr.has((-1) ** (n - 1)) or
            expr.has(sp.sin(n)) or expr.has(sp.cos(n))
        )
        ...
        if not has_fact:
            try:
                x = sp.Symbol("x", positive=True)
                c, p = abs_n.subs(n, 1 / x).leadterm(x)
                if not c.has(sp.O) and not p.has(sp.O) and p.is_number:
                    if not c.has(x):
                        if p > 1:
                            ...
                            if p == 1:
                                prof.stop("Asymp-p-test")
                                return False, "Divergent (Asymptotic Harmonic p=1)", prof.get_log_string()
                    else:
                        # Mixed log-polynomial term
                        if p > 1:
                            prof.stop("Asymp-p-test")
                            return True, f"Absolutely Convergent (Asymp p'={(1+p)/2} > 1)", prof.get_log_string()
                        elif not is_oscillatory and p < 1:
                            prof.stop("Asymp-p-test")
                            return False, f"Divergent (Asymp p'={(1+p)/2} < 1)", prof.get_log_string()
            except Exception:
                pass
        prof.stop("Asymp-p-test")
            ...
            try:
                ...
                if L_la is not None and L_la.is_number and not L_la.has(sp.Limit):
                    if L_la > 1:
                        ...
                        return True, f"Absolutely Convergent (Log-Asymp p={L_la} > 1)", prof.get_log_string()
                    elif not is_oscillatory and L_la < 1:
                        prof.stop("Log-Asymp-Test")
                        return False, f"Divergent (Log-Asymp p={L_la} < 1)", prof.get_log_string()
            except Exception:
                pass
        prof.stop("Log-Asymp-Test")
        ...
        if not has_fact and abs_n.has(sp.log):
            ...
            for level in range(1, 3):
                ...
                try:
                    x = sp.Symbol("x", positive=True)
                    c, p = current.subs(n, 1 / x).leadterm(x)
                    if not c.has(sp.O) and not p.has(sp.O) and p.is_number:
                        if not c.has(x):
                            if p > 1:
                                prof.stop("Cauchy-Condensation")
                                return True, f"Absolutely Convergent (Condensation L{level} p={p} > 1)", prof.get_log_string()
                            elif not is_oscillatory:
                                if p < 1:
                                    prof.stop("Cauchy-Condensation")
                                    return False, f"Divergent (Condensation L{level} p={p} < 1)", prof.get_log_string()
                                if p == 1:
                                    prof.stop("Cauchy-Condensation")
                                    return False, f"Divergent (Condensation L{level} p=1)", prof.get_log_string()
                        else:
                            if p > 1:
                                prof.stop("Cauchy-Condensation")
                                return True, f"Absolutely Convergent (Condensation L{level} p'={(1+p)/2} > 1)", prof.get_log_string()
                            elif not is_oscillatory and p < 1:
                                prof.stop("Cauchy-Condensation")
                                return False, f"Divergent (Condensation L{level} p'={(1+p)/2} < 1)", prof.get_log_string()
                except Exception:
                    pass
        prof.stop("Cauchy-Condensation")
        ...
        if (not has_fact
                ...
                and any(
                    isinstance(a, sp.Float) or
                    (isinstance(a, sp.Rational) and a != int(a))
                    for a in abs_n.atoms(sp.Number)
                )):
            try:
                x_sym     = sp.Symbol("x", positive=True)
                integrand = abs_n.subs(n, x_sym)
                result    = sp.integrate(integrand, (x_sym, 3, sp.oo))
                if result is not None and result.is_number and not result.has(sp.Limit):
                    if result.is_finite and result >= 0:
                        ...
                        return True, "Absolutely Convergent (Integral Test)", prof.get_log_string()
                    elif result == sp.oo:
                        prof.stop("Integral-Test")
                    ...
                    if ratio_limit == 1:
                        ...
                        try:
                            ...
                            if p == 1 and not c.has(x):
                                ...
                                prof.stop("Ratio-Test")
                                return False, "Divergent (Gauss/Raabe h=0 <= 1)", prof.get_log_string()
                        except Exception:
                            pass
            except Exception:
                pass
            ...
            try:
                ...
                if not c.has(sp.O) and not p.has(sp.O) and p.is_number:
                    if not c.has(x):
                        ...
                        if p > 1:
                            prof.stop("Asymp-Stirling")
                            return True,  f"Absolutely Convergent (Stirling p'={(1+p)/2} > 1)", prof.get_log_string()
                        elif not is_oscillatory and p < 1:
                            prof.stop("Asymp-Stirling")
                            return False, f"Divergent (Stirling p'={(1+p)/2} < 1)", prof.get_log_string()
            except Exception:
                pass
        prof.stop("Asymp-Stirling")
...
def main():
    ...
    sequences = [
        ...
        (sp.factorial(n) / 100**n,
         ...
         lambda k: math.exp(math.lgamma(k+1) - k*math.log(100)) if k < 500 else float("inf")),
        ((n / sp.log(n)) * (n**(1/n) - 1),
         "n/ln(n) * (n^(1/n) - 1)",
         lambda k: (k/math.log(k))*(k**(1/k)-1)),
        (sp.sqrt(n**2 + n) - n,
         "sqrt(n^2 + n) - n",
         lambda k: math.sqrt(k**2+k)-k),
        ((1 + 1/n)**(n**2),
        ...
        (sp.log(n)**sp.log(n) / n,
         ...
         lambda k: math.exp(math.log(math.log(k))*math.log(k) - math.log(k)) if k > 2 else 1),
        ((-1)**n * (n / (n + 1)),
        ...
        (((2**(4*n) * sp.factorial(n)**4) / (sp.factorial(2*n)**2 * (2*n + 1))),
         ...
         lambda k: math.exp(4*k*math.log(2)+4*math.lgamma(k+1)-2*math.lgamma(2*k+1)-math.log(2*k+1))),
        ...
        (sp.gamma(n + 0.5) / (sp.sqrt(n) * sp.gamma(n)),
         ...
         lambda k: math.exp(math.lgamma(k+0.5)-0.5*math.log(k)-math.lgamma(k))),
    ...
    series = [
        ...
        (sp.factorial(n) / n**n, 1,
         ...
         lambda k: math.exp(math.lgamma(k+1)-k*math.log(k))),
        ((sp.factorial(n) * sp.exp(n)) / n**(n+0.5), 1,
         "n!*e^n / n^(n+0.5) (Gauss)",
         lambda k: math.exp(math.lgamma(k+1)+k-(k+0.5)*math.log(k))),
        (sp.factorial(2*n) / (sp.factorial(n)**2 * 4**n), 1,
         "Wallis Diverge",
         lambda k: math.exp(math.lgamma(2*k+1)-2*math.lgamma(k+1)-k*math.log(4))),
        (sp.log(n)**sp.log(n) / n**sp.log(n), 2,
         "ln(n)^ln(n) / n^ln(n)",
         lambda k: math.exp(math.log(math.log(k))*math.log(k)-math.log(k)**2) if k > 2 else 1),
        (1 / (n**(1 + 1/sp.log(n))), 2,
        ...
        (1 / (n * sp.log(n) * sp.log(sp.log(n))**2), 3,
         "1/(n*ln(n)*ln(ln(n))^2)",
         lambda k: 1/(k*math.log(k)*math.log(math.log(k))**2) if k > 2 else 0),
        ((n**(n + 1/n)) / ((n + 1/n)**n), 1,
         "n^(n+1/n) / (n+1/n)^n (Heavy Base)",
         lambda k: math.exp((k+1/k)*math.log(k)-k*math.log(k+1/k))),
        (1 / (n**(sp.S(10001)/10000)), 1,
         "1 / n^(1.0001) (Poly Edge Trap)",
         lambda k: 1/k**1.0001),
        (sp.log(n)**sp.log(n) / 10**n, 2,
         "ln(n)^ln(n) / 10^n (Root Extractor)",
         lambda k: math.exp(math.log(math.log(k))*math.log(k)-k*math.log(10)) if k > 2 else 1),
    ]

    # ── Print sequences ───────────────────────────────────────────
    print(f"{Fore.CYAN}{Style.BRIGHT}{'='*155}")
    print(f"{Fore.CYAN}{Style.BRIGHT}{'BRUTAL MATH ENGINE v11.0 (ULTIMATE MERGED EDITION) — SEQUENCE TESTS':^155}")
    print(f"{Fore.CYAN}{Style.BRIGHT}{'='*155}")
    ...
    for i, (expr, desc, py_func) in enumerate(sequences, 1):
        ...
        total_seq_time += ms
        print(f"{i:<3} | {desc:<42} | {format_result(is_conv)} | {ms:>5.1f} ms | {reason:<36} | {Fore.LIGHTBLACK_EX}{logs}{Style.RESET_ALL}")

    # ── Print series ──────────────────────────────────────────────
    print(f"\n{Fore.MAGENTA}{Style.BRIGHT}{'='*155}")
    print(f"{Fore.MAGENTA}{Style.BRIGHT}{'BRUTAL MATH ENGINE v11.0 (ULTIMATE MERGED EDITION) — SERIES TESTS':^155}")
...
def second_main():
    ...
    sequences = [
        ...
        ((sp.factorial(2*n))**(sp.S(1)/n) / (4**n / n),
         "(2n!)^(1/n) / (4^n/n)  -> 0",
         lambda k: math.exp(math.lgamma(2*k+1)/k - k*math.log(4) + math.log(k))),
        (sp.factorial(2*n) / (sp.factorial(n) * (2*n)**(n + sp.S(1)/2)),
         "(2n)!/(n!*(2n)^(n+1/2)) Stirling",
         lambda k: math.exp(math.lgamma(2*k+1)-math.lgamma(k+1)-(k+0.5)*math.log(2*k))),
        (sp.gamma(n + sp.S(3)/2) / (sp.sqrt(n) * sp.gamma(n + 1)),
         "Gamma(n+3/2)/(sqrt(n)*n!)  -> 1",
         lambda k: math.exp(math.lgamma(k+1.5)-0.5*math.log(k)-math.lgamma(k+1))),
        (sp.sin(n * sp.pi / 2) / n,
         "sin(nπ/2)/n  -> 0",
         None),
        (n * sp.sin(sp.pi / n),
        ...
        (sp.log(n)**sp.log(sp.log(n)) / n,
         "ln(n)^ln(ln(n)) / n  -> 0",
         lambda k: math.exp(math.log(math.log(k))*math.log(math.log(k))-math.log(k)) if k > 2 else 1),
        ...
        ((2*n * sp.factorial(n))**2 / sp.factorial(2*n+1),
         "(2n*n!)^2 / (2n+1)!  -> 0",
         lambda k: math.exp(2*math.log(2*k)+2*math.lgamma(k+1)-math.lgamma(2*k+2))),
    ...
    # ── Series ────────────────────────────────────────────────────
    series = [
        (1 / (n * sp.log(n) * sp.log(sp.log(n))), 3,
         "1/(n*ln(n)*ln(ln(n))) Div",
         lambda k: 1/(k*math.log(k)*math.log(math.log(k))) if k > 2 else 0),
        (1 / (n**(sp.S(1) + sp.S(1)/n)), 2,
         "1/n^(1+1/n) Div (-> harmonic)",
         lambda k: 1/k**(1+1/k)),
        (1 / (n * sp.log(n)**2 * sp.log(sp.log(n))), 3,
         "1/(n*ln^2(n)*ln(ln(n))) Conv",
         lambda k: 1/(k*math.log(k)**2*math.log(math.log(k))) if k > 2 else 0),
        ...
        (sp.factorial(3*n) / (sp.factorial(n) * sp.factorial(2*n) * 3**n), 1,
         "(3n)!/(n!(2n)!3^n) Div",
         lambda k: math.exp(math.lgamma(3*k+1)-math.lgamma(k+1)-math.lgamma(2*k+1)-k*math.log(3))),
        ((sp.log(n))**n / n**n, 2,
        ...
        (sp.log(n)**sp.log(n) / n**2, 2,
         ...
         lambda k: math.exp(math.log(math.log(k))*math.log(k)-2*math.log(k)) if k > 2 else 1),
        (sp.log(n)**n / sp.factorial(n), 1,
         "ln(n)^n / n!  Conv (ratio->0)",
         lambda k: math.exp(k*math.log(math.log(k))-math.lgamma(k+1)) if k > 1 else 0),
        (1 / n**(sp.S(1) + sp.sin(sp.S(1)/n)), 1,
         "1/n^(1+sin(1/n)) Div",
         lambda k: 1/k**(1+math.sin(1/k))),
        (sp.factorial(n) * sp.factorial(n) / sp.factorial(2*n + 1), 1,
         "n!*n!/(2n+1)! Conv",
         lambda k: math.exp(2*math.lgamma(k+1)-math.lgamma(2*k+2))),
        ((4*n**2) / (4*n**2 - 1), 1,
         "Wallis product terms Div (-> 1)",
         lambda k: (4*k**2)/(4*k**2-1)),
        (1 / (n * sp.log(n)**(sp.S(3)/2)), 2,
         "1/(n*ln(n)^1.5) Conv",
         lambda k: 1/(k*math.log(k)**1.5)),
        (1 / (n * sp.log(n) * sp.log(sp.log(n))**(sp.S(1)/2)), 3,
         "1/(n*ln*ln(ln)^0.5) Div",
         lambda k: 1/(k*math.log(k)*math.log(math.log(k))**0.5) if k > 2 else 0),
        (sp.exp(-sp.sqrt(n)), 1,
         "e^(-sqrt(n))  Conv",
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}{'BRUTAL MATH ENGINE v11.0 (LETHAL EDITION) — SEQUENCE TESTS':^155}")
    print(f"{Fore.CYAN}{Style.BRIGHT}{'='*155}")
    print(f"{'No.':<3} | {'Description':<42} | {'Result':<10} | {'Time':<8} | {'Details':<36} | {'Profiler Logs'}")
    print("-" * 155)
    ...
    # ── Print series ──────────────────────────────────────────────
    print(f"\n{Fore.MAGENTA}{Style.BRIGHT}{'='*155}")
    print(f"{Fore.MAGENTA}{Style.BRIGHT}{'BRUTAL MATH ENGINE v11.0 (LETHAL EDITION) — SERIES TESTS':^155}")
    print(f"{Fore.MAGENTA}{Style.BRIGHT}{'='*155}")
    print(f"{'No.':<3} | {'Description':<42} | {'Result':<10} | {'Time':<8} | {'Details':<36} | {'Profiler Logs'}")
    print("-" * 155)
