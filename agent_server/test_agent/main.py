...
async def websocket_endpoint(websocket: WebSocket):
    ...
    logger.info("Session %s created for %s (%.0fms)", session.id, user_id,
        ...
        while True:
            ...
            try:
                async for event in runner.run_async(
                    ...
                    if first_text_event:
                        logger.info("[%s] First text event in %.1fs",
                                    user_id, time.monotonic() - t_start)
                ...
                logger.info("[%s] Agent run completed in %.1fs",
                            user_id, time.monotonic() - t_start)

            except Exception as e:
                logger.error("[%s] Error during agent run: %s", user_id, e, exc_info=True)
                try:
                    await websocket.send_json({
                        "type": "error",
